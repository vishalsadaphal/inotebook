...
const JWT_SECRET = 'vishalisgoodboy';
...
router.post('/login',[
    ...
    body('email', 'enter a valid email').isEmail(),
    body('password', 'password can not be blane').exists(),
...
try {
    ...
    if(!user){
        success: false
        return res.status(400).json({error: "Pleasee try to login wiith correct credentials"});
    }
